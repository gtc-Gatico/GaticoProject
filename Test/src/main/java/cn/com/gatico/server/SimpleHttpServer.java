...
public class SimpleHttpServer {
    public static Logger logger = LoggerFactory.getLogger(SimpleHttpServer.class);
    public static String uri = "/index.html";

    public static void Start(Integer port) {
        try {
            HttpServer httpServer = HttpServer.create(new InetSocketAddress(port), 60);
            httpServer.createContext("/", SimpleHttpServer::handleRequest);
...
/*        ServerSocket serverSocket;
                        ...
                        if ("POST".equals(requestTemp.getMethod()) && ((contentType = (String) requestTemp.getHeads().get("Content-Type")) != null
                            ...
                            while ((str = bufferedReader.readLine()) != null) {
                                ...
                                if (str.indexOf("Content-Disposition:") >= 0 && str.indexOf("filename") > 0) {
                                    ...
                                    //这一行是Content-Type
                                    String fileContentType = bufferedReader.readLine();
                                    bl += (fileContentType + "\r\n").length();
                                    bufferedReader.readLine();
                                    bl += "\r\n".length();
                                    System.out.println("contentType = " + fileContentType);
                                    char[] arr = new char[contentLength - bl];
                                    bufferedReader.read(arr);
                            ...
                            bodyLineStr = bufferedReader.readLine();*/
                        // ApplicationContext.invoke(requestTemp, socket.getOutputStream());
                    }
  ...
  /*      new Thread(new Runnable() {
        ...
        try {
            Request requestTemp = new Request();
            requestTemp.setMethod(exchange.getRequestMethod());
            requestTemp.setPath(exchange.getRequestURI().toString());
            requestTemp.setVersion(exchange.getProtocol());
            requestTemp.setUrl(exchange.getRequestURI().getPath());
            if (requestTemp.getPath().equals("/")) {
                requestTemp.setUrl(uri);
            }

            if (exchange.getRequestURI().getQuery() != null && exchange.getRequestURI().getQuery().length() > 0) {
            ...
            requestTemp.getHeads().putAll(exchange.getRequestHeaders());

            if (requestTemp.getMethod().equals("POST")) {
                exchange.getHttpContext().getAttributes().forEach((s, o) -> {
                    System.out.println(s);
                });
            }
            logger.info(requestTemp.getMethod() + "：" + requestTemp.getPath());
            ApplicationContext.invoke(requestTemp, exchange);

        } catch (Exception ex) {
            logger.error("SimpleHttpServer", ex);
            Response response = new Response();
            response = response.getError();
            File file = new File(ApplicationContext.staticPath + ApplicationContext._500Path);
            if (!file.exists()) {
                try {
                    ...
                    String htmlTemplate = html.replace("${errormsg}", errorStr);
                    File tempFile = File.createTempFile("500", ".html");
                    ex.printStackTrace(new PrintWriter(tempFile));
                    FileWriter fileWriter = new FileWriter(tempFile);
                    fileWriter.write(htmlTemplate);
                    fileWriter.flush();
                    file = tempFile;
            ...
            ApplicationContext.writerFile(file, response, exchange);
        }
/*            }
        }).start();*/
    }
