...
public class TestRRD {
    ...
    public String createRrd(DataEntity dataEntity,long end) {
        try {
            this.dataEntity = dataEntity;
            //设置创建时间（unix时间戳）
            long start = dataEntity.getTime();
            long day=(end-start)/(3600*24);
...
//        rrdDef.addDatasource("class", DsType.GAUGE, 600, 0, Double.NaN);
    ...
    public String addOrUpdateRrdResource(DataEntity dataEntity) {
    ...
    public String showRrdGraph(String VerticalLabel,String imgPath, long startTime, long endTime)  {
        ...
        gDef.datasource("Class", file_path, "class", ConsolFun.AVERAGE);*/
            ...
            for (int i = 0; i < dataEntity.getDataSources().length; i++) {
                gDef.line(dataEntity.getDataSources()[i], color[i], dataEntity.getDataSources()[i]);
            }
