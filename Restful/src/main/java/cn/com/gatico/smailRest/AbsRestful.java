...
public class AbsRestful implements HttpHandler {
    ...
    public Map<String, UrlClass> urlMap = new ConcurrentHashMap<>();
    public String encoding = System.getProperty("restful.encoding").length() > 0 ? System.getProperty("restful.encoding") : "UTF-8";
    ...
    public void handle(HttpExchange httpExchange) throws IOException {
        ...
        try {
            ...
            if (returnValue instanceof byte[]) {
                httpExchange.getResponseBody().write((byte[]) returnValue);
            } else {
    ...
    private void setContextType() {
        ...
        if (!httpExchange.getResponseHeaders().containsKey("Content-Type")) {
            httpExchange.getResponseHeaders().set("Content-Type", "application/json;charset=" + encoding);
        }
    }
}
